...
<style>
  /* Tighter than before on purpose: the quiz card now sits inside the hero and
     has to be visible on a phone without scrolling. */
  .pf-hero {{ padding: clamp(1.5rem, 4vw, 2.5rem) 0 clamp(2rem, 5vw, 3rem); text-align: center; }}
  .pf-hero h1 {{ font-family: Zodiak, Georgia, serif; font-weight: 700;
    font-size: clamp(1.85rem, 5.4vw, 3rem); letter-spacing: -.02em; max-width: 22ch;
    margin: .6rem auto 0; }}
  .pf-lede {{ max-width: 46ch; margin: .9rem auto 0; font-size: clamp(1rem, 2.2vw, 1.1rem);
    opacity: .82; }}
  ...
  .qz {{ max-width: 34rem; margin: 1.6rem auto 0; }}
  ...
  /* The card centres its own copy; these rows are two-column and read as a
     ragged mess when each line starts in a different place. */
...
<main id="top">
  ...
  <!-- The quiz is the hero. It used to sit in section two behind a "skip to the
       questions" link, so the first screen was a headline and a price; ~195 ad
       arrivals produced 5 onward clicks. The first thing on the page is now the
       thing that shows people they have the problem, and the only ask above the
       fold is answering a question about their own pet. Both purchase CTAs live
       on the quiz result card (quiz.js) and in the "One price, once" section. -->
  <section class="pf-hero" id="questions" aria-labelledby="q-h">
    <div class="wrap">
